...
yolo_class_index_to_open_images_lable_name={}
...
with open(dataset_config['path']+'/train/metadata/classes.csv', 'r') as open_images_v7_metadata:
  for line in open_images_v7_metadata:
        ...
        if match:
           yolo_class_index_to_open_images_lable_name[inverted_yolov8_class_map[match.group(2)]]=match.group(1)
  ...
  print(yolo_class_index_to_open_images_lable_name)
  print(detections_label_name_subexpression)
...
#detections_expression = '(' + detections_expression +  '),.+,('+detections_label_name_subexpression+'),(0\.\d+),(0\.\d+),(0\.\d+),(0\.\d+)'

detections_expression = '(' + detections_expression +  '),.+,('+detections_label_name_subexpression+'),(0\.\d+),(0\.\d+),(0\.\d+),(0\.\d+)'

print(detections_expression)

with open(dataset_config['path']+'/train/labels/detections_subset.csv', 'r') as open_images_v7_detections:
